...
namespace UnityEngine.Networking
{
	...
	public class NetworkManagerHUD : MonoBehaviour
	{
		...
		int offsetX = Screen.width/5;
		int offsetY = Screen.height - 300;
		...
		void OnGUI()
		{
			...
			if (!NetworkClient.active && !NetworkServer.active && manager.matchMaker == null)
			{
				if (GUI.Button(new Rect(xpos, ypos, 300, 100), "Create Server(H)",myStyle ))
				{
					manager.StartHost();
				}
				ypos += spacing;

				if (GUI.Button(new Rect(xpos , ypos, 300, 100), "Join Server(C)", myStyle))
				{
					manager.StartClient();
				}
				//manager.networkAddress = GUI.TextField(new Rect(xpos + 150, ypos, 95, 40), manager.networkAddress);
				//ypos += spacing;
                //
			...
			if (NetworkServer.active || NetworkClient.active)
			{
				//if (GUI.Button(new Rect(xpos, ypos, 200, 40), "Stop (X)"))
				//{
				//	manager.StopHost();
				//}
				//ypos += spacing;
